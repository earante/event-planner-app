import React, { useState, useEffect } from 'react';
    ...
    const SignUpPage = () => {
      ...
      const handleSubmit = async (e) => {
        ...
        if (success) {
          navigate('/dashboard');
           toast({ title: "Signup Successful", description: "Welcome!", variant: "default" });
        } else {
           toast({ title: "Signup Failed", description: "Email already exists or another error occurred.", variant: "destructive" });
        }
      };
        ...
        >
          <Card className="w-full max-w-md glassmorphism shadow-2xl border-border/50">
            <CardHeader className="text-center bg-card/70 p-8 rounded-t-lg">
              ...
              <CardDescription className="text-muted-foreground">Join Event Planner and start organizing!</CardDescription>
            </CardHeader>
            ...
            <CardFooter className="p-6 text-center bg-card/50 rounded-b-lg border-t border-border/50">
              <p className="text-sm text-muted-foreground">
                ...
                <motion.div whileHover={{ scale: 1.05 }} className="inline-block">
                  <Link to="/login" className="font-semibold text-accent hover:text-accent/90 underline">
                    Sign In
                  </Link>
                </motion.div>
              </p>
